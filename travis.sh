...
# Get versions
DEMYX_ALPINE_VERSION="$(docker exec ssh cat /etc/os-release | grep VERSION_ID | cut -c 12- | sed -e 's/\r//g')"
DEMYX_OPENSSH_VERSION="$(docker exec ssh ssh -V | awk -F '[,]' '{print $1}' | cut -c 9- | sed -e 's/\r//g')"
